...
class Boxes_Encode(nn.Module):
    ...
    def spo_boxes(self, boxes, rel_inds, part_boxes):
        s_boxes = boxes[rel_inds[:, 0]]

        if part_boxes is not None:
            o_boxes = part_boxes[rel_inds[:, 1]]
        else:
    ...
    def forward(self, boxes, rel_inds, width, height, part_boxes=None):
        s_boxes, o_boxes, u_boxes = self.spo_boxes(boxes, rel_inds, part_boxes)
